...
```
앱 실행시, 가장 먼저 나타나는 Activity로 아이디와 비밀번호를 입력해 사용자의 신원을 확인한다. 다른 앱들과 동일하게 처음 로그인 접속할 때를 제외하고는 자동 로그인과 같이 접속방식을 사용자가 제어 할 수 있다.다른 앱들과 동일하게 인증절차를 담당하고 있는 부분입니다.
